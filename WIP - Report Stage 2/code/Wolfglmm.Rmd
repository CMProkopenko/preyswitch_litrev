...
studyarea <- StudyArea_Nov2025 %>%
 ...
 Biome_Dinerstein_Name = case_when(
    Biome_Dinerstein_Name == "Temperate Broadleaf & Mixed Forests" ~ "TempBroadMixFor",
    Biome_Dinerstein_Name == "Temperate Grasslands, Savannas & Shrublands" ~ "TempGrassSavShrub",
    Biome_Dinerstein_Name == "Boreal Forests/Taiga" ~ "BorealForestTaiga",
    Biome_Dinerstein_Name == "Mediterranean Forests, Woodlands & Scrub" ~ "MediForScrub",
    Biome_Dinerstein_Name == "Temperate Conifer Forests" ~ "TempConiferFor",
    Biome_Dinerstein_Name == "Montane Grasslands & Shrublands" ~ "MontGrassShrub",
    Biome_Dinerstein_Name == "Tundra" ~ "Tundra",
    Biome_Dinerstein_Name == "Deserts & Xeric Shrublands" ~ "DesertXericShrub",
    TRUE ~ Biome_Dinerstein_Name
  )) %>%
  select(Study_Code, Study_Country, Realm_Olson, Realm_OneEarth, Biome_Olson, Biome_Dinerstein, Biome_Dinerstein_Name, Bioregion_Olson, Bioregion_OneEarth, Eco_Code_Olson, Eco_Num_Olson, Ecoregion_Olson, Suprarealm_OneEarth, Subrealm_OneEarth, Ecoregion_OneEarth, Ecoregion_OneEarth2)
...
scatstudy.unfinished <- scatstudy.unfinished %>%
  ...
  Biome_Dinerstein = as.factor(Biome_Dinerstein),
  Biome_Dinerstein_Name = as.factor(Biome_Dinerstein_Name),
...
```{r}
modelw.kga.mc.wun.def.ecooe <- glm(Prop_kg_Cons ~ 0 + Prop_kg_Avail + Mass_Cat + Wild_Ung_Avail_n + Defense + Biome_Dinerstein_Name, data= scatstudy.4mw) #Proportion biomass consumed ~ 0 + Proportion biomass available + body size category + number of wild ungulate prey species + antipredator + biome
