...
group_by(Study_Code, Period) %>%
  ...
  ungroup() %>%
         ...
         mutate(Prop_Ind = ifelse(is.na(Num_Ind), Prop_Ind_Avail/Prop_Ind_Avail_Sum, ifelse(!is.na(Num_Ind) & Ind_Tot>0, Num_Ind / Ind_Tot, NA_real_)),
         # Ratio of individuals available
         Pre_Ratio_Ind = ifelse(is.na(Num_Ind), 1 - Prop_Ind, ifelse(!is.na(Num_Ind) & !is.na(Ind_Tot), Ind_Tot - Num_Ind, ifelse(!is.na(Density) & !is.na(Density_Tot), Density_Tot - Density, NA_real_))),
    Ratio_Ind = case_when(
      !is.na(Num_Ind)  & !is.na(Pre_Ratio_Ind) & Pre_Ratio_Ind > 0 ~ Num_Ind  / Pre_Ratio_Ind,
      !is.na(Prop_Ind) & !is.na(Pre_Ratio_Ind) & Pre_Ratio_Ind > 0 ~ Prop_Ind / Pre_Ratio_Ind,
      !is.na(Density)  & !is.na(Pre_Ratio_Ind) & Pre_Ratio_Ind > 0 ~ Density  / Pre_Ratio_Ind,
      TRUE ~ NA_real_
    )
  ) %>%
  group_by(Study_Code, Period, Prey_Taxa_Valid) %>%
...
#Calculate the proportion and ratio of biomass consumed
  ...
  mutate(Pre_Ratio_kg_Cons = Tot_kg_Cons - Prey_Tot_kg_Cons) %>%
  mutate(Ratio_kg_Cons = Prey_Tot_kg_Cons/Pre_Ratio_kg_Cons) %>%
  relocate(Mass_Cat, Ind_Tot, Density_Tot, Prop_Ind, Ratio_Ind, Pre_Ratio_Ind, Prop_kg_Avail, Ratio_kg_Avail, .before = Orig_BM_Avail) %>%
...
scattop2.nomixedgroup <- scattop2 %>%
  filter(!Prey_Item %in% ("Cervids")) %>%
   group_by(Study_Code, Period) %>%
  filter(n() == 2) %>%
  ungroup()
