...
#Calculate the proportion and ratio of biomass consumed
  ...
  mutate(Pre_Ratio_kg_Cons = Tot_kg_Cons - Prey_Tot_kg_Cons) %>%
  mutate(Ratio_kg_Cons = Prey_Tot_kg_Cons/Pre_Ratio_kg_Cons) %>%
  relocate(Mass_Cat, Ind_Tot, Density_Tot, Prop_Ind, Ratio_Ind, Prop_kg_Avail, Ratio_kg_Avail, .before = Orig_BM_Avail) %>%
...
```{r}
modelw.ratio.ind.occ.scattop2 <- glm(Ratio_Occs ~ 0 + Ratio_Ind + Mass_Cat +  Defense + Biome_Detal_Name, data= scattop2) #Proportion biomass consumed ~ 0 + Proportion biomass available + body size category +  antipredator + biome
summary(modelw.ratio.ind.occ.scattop2)
```


```{r}
scattop2.ratios <- scattop2 %>%
  filter(Ratio_kg_Avail < 20)

scattop2_ratio_cons_avail_plot <- ggplot(scattop2.ratios) +
...
ggsave("~/Github/preyswitch_litrev/WIP - Report Stage 2/plots/scattop2_ratio_cons_avail_plot.png", plot = scattop2_ratio_cons_avail_plot, height = 5, width = 7, dpi = 700)

scattop2_ratio_cons_avail_plot
```
